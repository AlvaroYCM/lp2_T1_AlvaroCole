...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void listar() {
		EntityManager manager = JPAUtil.getEntityManager();
		String jpql = "select e from OrdenDental  e";

		try {
			List<OrdenDental> lstOrdenes = manager.createQuery(jpql, OrdenDental.class).getResultList();

			for (OrdenDental ordenDental : lstOrdenes) {
				Dentista dentista = ordenDental.getDentista();
			

				imprimir("Nro equipo......: " + ordenDental.getNroEquipo());
				imprimir("Nombre.: " + ordenDental.getNombre());
				imprimir("Costo...........: " + ordenDental.getCosto());
				imprimir("Fecha de adquisicion.....: " + ordenDental.getFecha_adquisicion());
				imprimir("Monto.............: " + "S/. " + dentista.getNombre_completo());
				imprimir("Correo: " + dentista.getCorreo());
				imprimir("Especialidad: " + dentista.getId_especialidad());
				imprimir("Estado: " + ordenDental.getestado());
				imprimir("-------------------------------------\n");
			}
	...
	void adicionar() {
		...
		try {
			OrdenDental ordenDental = new OrdenDental(null, Dentista, nombre, costo, null, estado);
			
			manager.getTransaction().begin();
			manager.persist(ordenDental);
			manager.getTransaction().commit();
			
			mensajeInfo("Orden de equipo registrada");
			limpiar();
	...
	void buscar() {
		...
		Integer nroOrden = Integer.parseInt(txtNroEquipo.getText());
		EntityManager manager = JPAUtil.getEntityManager();
		
		try {
			OrdenDental ordenSoporte = manager.find(OrdenDental.class, nroOrden);
			...
			txtNroEquipo.setText(ordenSoporte.getNroEquipo()+"");
			txtNombre.setText(ordenSoporte.getNombre());
			txtCosto.setText(ordenSoporte.getCosto()+"");
			cboEstados.setSelectedItem(ordenSoporte.getestado());
			txtFechaAdquisicion.setText(ordenSoporte.getFecha_adquisicion()+"");
	...
	void modificar() {
		...
		Double costo = Double.parseDouble(txtCosto.getText());
		Dentista dentista = (Dentista) cboDentistas.getSelectedItem();
		LocalDate fechaRegistro = LocalDate.parse(txtFechaAdquisicion.getText()); 
		String estado = (String)cboEstados.getSelectedItem();
		...
		try {
			...
			manager.getTransaction().begin();
			manager.merge(ordenDental);
			manager.getTransaction().commit();
			
			mensajeInfo("Orden de equipo actualizada");
			limpiar();
	...
	void eliminar() {
		...
		try {
			OrdenDental ordenDental = manager.find(OrdenDental.class, nroOrden);
			
			if (ordenDental == null) {
				mensajeAdvertencia("Orden de equipo no encontrada");
				return;
			}
			
			manager.getTransaction().begin();
			manager.remove(ordenDental);
			manager.getTransaction().commit();
			
			mensajeInfo("Orden de equipo eliminada");
			limpiar();
		} catch (Exception e) {
			mensajeError("Hubo un error en la transacción");
			e.printStackTrace();
